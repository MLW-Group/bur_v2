...
import { ModalContainer, Container, RowCenter, ModalBody, ModalHeader, Close, BodyModal } from './styled';
...
const Modal = ({
	...
	const { typeModal, paramsModal } = useContext(AppContext);
	const [phone, setPhone] = useState('');
  const wellOptions = paramsModal
	const sendForm = async () => {
		if (phone.length == 0 || phone.includes('_')) {
			...
			await axios.post(
				...
				// `http://192.168.1.216:3500/api/v1/request/create/`,
				{
					phone,
					modelType: typeModal,
					wellOptions,
	...
	return (
		<ModalContainer>
			<Container>
              ...
              /> */}
							{error && (
								<Text $size="M" $fontWeight="L" $color="error" $fontFamily="open">
									{error}
								</Text>
							)}
							<Button
