...
const ModeratorTable: React.FC = () => {
	...
	const getAllUsers = async () => {
		try {
			const { data } = await axios.get(`https://bur-api.macwel.app/api/v1/user`, {
	...
	return (
		<>
			...
			>
				<Button type="primary" onClick={() => setCreateUser({ ...createUser, active: true })}>
					...
					{(props) => {
						...
						return (
							<Form>
								...
								/>
								<p style={{ color: 'red', marginBottom: 10 }}>{errors.password}</p>
								{me.role === 'ADMIN' ? (
									<>
