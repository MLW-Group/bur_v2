...
const EditableCell: React.FC<EditableCellProps> = ({
	...
	const inputNode = dataIndex === 'colorLabel' ? <Select options={colorsMapping} /> : <Input />;
...
export default function TableMarks({ marks, deleteMark, getAllMarks, token, setMarksLocation }) {
		...
		},
		{
			...
			render: (_: any, record: Item) => {
				...
				return editable ? (
					...
					<span>
						<Typography.Link onClick={() => setMarksLocation([record.latitude, record.longitude, 18])}>
							Показать
						</Typography.Link>
						<Typography.Link style={{ marginLeft: 10 }} disabled={editingKey !== ''} onClick={() => edit(record)}>
							Изменить
						</Typography.Link>
